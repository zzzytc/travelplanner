...
def flight_node(messages):
    query = query_chain.invoke({
        "messages": [
            messages[0].content + 
            " Construct a concise search query to find flights based on the origin and destination using a search engine."
        ]
    })
    ...
    prompt = (
        "You are given real-time flight search results retrieved from a web search. "
        "The content may be in semi-structured JSON or HTML format. "
        "Please analyze the results and suggest the top 1–3 best flight options for the user, "
        "including airline, departure and arrival times, stops, and price if available.\n\n"
        f"Raw browser result:\n{tavily_result}"
    )
...
def hotel_node(messages):
    query = query_chain.invoke({
        "messages": [
            messages[0].content + 
            " Construct a concise search query to find boutique hotel options for the trip using a search engine."
        ]
    })
    ...
    prompt = (
        "The following is real-time hotel data from search engine results. "
        "Please extract 2–3 real boutique hotel options based ONLY on the content provided. Do not invent hotels or guess missing information.\n\n"
        f"{tavily_result}\n\n"
        "For each option, include:\n"
        "- Hotel name\n- Location\n- Price per night\n- Amenities\n- Any standout features (e.g., view, location)\n\n"
        "Present this like a travel consultant report."
    )
...
def summary_node(messages: Sequence[BaseMessage]) -> List[BaseMessage]:
    ...
    prompt = (
    "Please summarize the complete travel plan for the user, including:\n"
    "- Trip title and overall travel dates\n"
    "- Traveler information (name, number of people, travel class, budget per person if available)\n"
    "- Flight details (departure/return, origin, destination, date, preferred airlines, and stop preferences)\n"
    "- Hotel or accommodation details (hotel name, city, check-in/check-out dates, star rating, amenities, view preferences, and budget if provided)\n"
    "- Daily itinerary highlights (date, location, activity name, time and description if available)\n"
    "- Budget alignment across flights, hotels, and activities if mentioned\n"
    "Present the plan clearly and helpfully as if you're a professional travel consultant summarizing it for a client."
    )
